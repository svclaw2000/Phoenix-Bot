...
{    
    ...
    public WeightedItemSet(ItemSet itemSet) {
        ...
        for (int i = 0; i < itemSet.items.size(); i++)
            add((Item) itemSet.items.elementAt(i), 1.);
    }
	...
	if (nbKnown == 0) { // No information can be gained from this test
	    Test test;
	    
	    if (testAttribute instanceof SymbolicAttribute)
		test = new SymbolicTest((SymbolicAttribute) testAttribute,
					new KnownSymbolicValue[] { 
					    new KnownSymbolicValue(0) });
	    else
		test = new NumericalTest((NumericalAttribute) testAttribute,
					 0.);
	    
	    return new TestScore(test, 0.);
	}
	
	TestScore knownTestScore = knownItems.bestSplitTest(testAttribute,
							    goalAttribute);
	
	return new TestScore(knownTestScore.test, 
			     knownTestScore.score * (double) nbKnown /
			     (double) items.size());
    }
	...
	for(int i = 0; i < sets.length; i++)
	    sets[i] = new WeightedItemSet(attributeSet);
	...
	for (int i = 0; i < items.size(); i++) {
		...
		sets[test.perform(value)].add(item, weight);
	}
	...
	for (int i = 0; i < unknownItems.size(); i++) {
	    ...
	    double weight = 
		((Double) unknownItemsWeights.elementAt(i)).doubleValue();	

	    for (int i2 = 0; i2 < sets.length; i2++)
		sets[i2].add(item, weight * setSizes[i2] / setSizeSum);
	}
