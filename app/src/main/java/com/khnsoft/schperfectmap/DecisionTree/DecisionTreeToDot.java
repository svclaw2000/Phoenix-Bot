...
public class DecisionTreeToDot {
	...
	private String jys_produceLabel(Node node) 
	{
		String label = "";
...
//			}
		...
		else /* Leaf */ 
		{
			...
			//label += "Leaf";
			label += leafNode.tree().getGoalAttribute().name();

			if (leafNode.goalValue() != null)
				if (tree.getGoalAttribute() != null)
					goalValueString = " - " + tree.getGoalAttribute().valueToString(leafNode.goalValue());
				else
